                            ...
                            <div className="col-12 col-lg-3 col-md-6" key={item.product.id}>
                                <div className="card" style={{width:'18rem'}}>
                                    ...
                                    <div className="card-body">
                                        ...
                                        <button className="btn btn-primary" onClick={() => dispatch(increaseQuantity(item.product.id))}>+</button>
                                        <span>Quantity: {item.quantity}</span>
                                        <button className="btn btn-primary" onClick={() => dispatch(decreaseQuantity(item.product.id))}>-</button>
                                        <button className="btn btn-primary" onClick={()=>dispatch(removeFromCart(item.product.id))}>Remove From Cart</button>
                                    </div>
                                </div>
                            </div>
                        ))}
